...
Config.load({
    ...
    .then(() => { // no carregamento bem-sucedido, inicie seu aplicativo
        ...
        app.get('/', function (req, res) {
            if ("dev" == profile) {
                return run.apply(req("http://dev-cor-backend.sura-aws.com/corporativo-backend-servicos"));
...
Config.load({
    ...
    .then(() => { // no carregamento bem-sucedido, inicie seu aplicativo
        ...
        app.get('/', function (req, res) {
            if ("hml" == profile) {
                return run.apply(req("http://hml-cor-backend.sura-aws.com/corporativo-backend-servicos"));
